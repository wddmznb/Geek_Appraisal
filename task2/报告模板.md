...
1. 当Huffman树的叶结点为n时，整个生成树的结点总数为何是2n-1？
   ...
   答：Huffman树是所谓的正则二叉树，只有度为0和度为2的结点 根据二叉树的性质，n0 = n2 + 1，因此该树中度为2结点数量为n-1 于是一共有2n-1个结点

2. 该代码可以对大写字母的字符串进行huffman编码，如果要读小写字母字符串进行编码，应该如何修改？如果想要对同时具有大写和小写字母的字符串都进行编码呢（写出思路即可）？

3. 使用画图工具画出上述两个Huff![微信图片_20221104222901](C:\Users\廖思远\Desktop\微信图片_20221104222901.jpg)man生成树的生成过程
