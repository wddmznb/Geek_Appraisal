...
int main(int argc, char *argv[])
{
    ...
    for(int i = 0; i < 27; i++)
    {
        if(arr_big[i] >= 1)
        {
            arr_small[count2++] = arr_big[i];	/*将出现过的字母的次数转移进arr_small*/
        }
    }
    ...
    {
        int length = 0;
        if(huffman[i][1] == -1 && huffman[i][2] == -1)  /*判断出该节点为叶节点*/
        {
...
void findMin(int ii[],int huffman[][4],int n)
{
    ...
    for(int i = 0; i < 2 * n - 1; i++)
    {
        if(huffman[i][3] == -1 && huffman[i][0] == -1)  /*该结点未赋上权值，则退出循环*/
        {
            break;
        }
        if(huffman[i][3] < min && huffman[i][0] == -1)  /*在循环中找到第一个权重最小的结点，将列数赋值给ii数组*/
        {
            min = huffman[i][3];
            ii[0] = i;
        }
    ...
    for(int j = 0;j<2 * n - 1;j++)
	{
		...
		if(huffman[j][3] < min && huffman[j][0] == -1 && j!=ii[0])
		{
			min = huffman[j][3];
			ii[1] = j;
		}
